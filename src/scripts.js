...
export const updateUserData = (id) => {
   Promise.all([customerData, roomData, bookingData]).then((data) => {
      allRooms = [];
      allBookings = [];
      hotel = "";
      currentUser = "";
      data[2].bookings.forEach((booking) =>
         allBookings.push(new Bookings(booking))
      );
      data[1].rooms.forEach((room) => allRooms.push(new Room(room)));
      data[0].customers.forEach((user) =>
         allUsers.push(new User(user, allRooms))
      );
...
const bookRoom = (currentUser, event, availableRooms) => {
   ...
   `;
   const date = new Date(checkBookings.value)
      .toISOString()
      .split("T")[0]
      .split("-")
      .join("/");
   const rooms = availableRooms.find(
      (room) => event.target.id == room.roomNumber
   );
   const bookingObject = {
   ...
   updateBookings(bookingObject).then(() => {
      updateUserData(id);
   });
};
...
logInButton.addEventListener("click", function (event) {
   ...
   if (
      username.value.slice(0, 8) === "customer" &&
      password.value === "overlook2021"
   ) {
